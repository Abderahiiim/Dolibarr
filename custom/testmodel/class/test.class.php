...
class Test extends CommonObject
{
	 ...
	 */
	public $table_element = 'testmodel_test';
	...
	/**
	 * @var string String with name of icon for test. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'test@testmodel' if picto is file 'img/object_test.png'.
	 */
	...
	//  */
	// public $table_element_line = 'testmodel_testline';
	...
	//  */
	// protected $childtablesoncascade = array('testmodel_testdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('testmodel', 'test', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('testmodel', 'test', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('testmodel', 'test_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->testmodel->dir_output.'/test/'.$oldref;
				$dirdest = $conf->testmodel->dir_output.'/test/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->testmodel->dir_output.'/test/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('testmodel','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('testmodel','testmodel_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'TESTMODEL_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'TESTMODEL_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'TESTMODEL_MYOBJECT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/testmodel/test_card.php', 1).'?id='.$this->id;
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('TESTMODEL_MYOBJECT_ADDON')) {
			$conf->global->TESTMODEL_MYOBJECT_ADDON = 'mod_test_standard';
		}

		if (getDolGlobalString('TESTMODEL_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('TESTMODEL_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('TESTMODEL_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/testmodel/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/testmodel/doc/";
