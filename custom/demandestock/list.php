...
$langs->loadLangs(array("demandestock@demandestock", "other"));
...
$object = new Demandestock($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->demandestock->dir_output.'/temp/massgeneration/'.$user->id;
...
// 	"object_demande"=> array("type"=> "varchar(30)", "label"=> $langs->trans("ObjectDemande"),'enabled' => 1, 'visible' => '1', "position" => 0 ),
...
// 	"fk_warehouse"=> array("type"=> "integer:Entrepot:product/stock/class/entrepot.class.php", "label"=> $langs->trans("Warehouse"),'enabled' => 1, 'visible' => '1', "position" => 0 ),
// 	"type_demande"=> array("type"=> "integer", "label"=> $langs->trans("TypeDemande"),'enabled' => 1, 'visible' => '1', "position" => 0 ),
// 	"desired_date"=> array("type"=> "date", "label"=> $langs->trans("DateSouhaite"),'enabled' => 1, 'visible' => '1', "position" => 0 ),
// 	"date_creation"=> array("type"=> "date", "label"=> $langs->trans("dateCreation"),'enabled' => 1, 'visible' => '1', "position" => 0 ),
// 	"fk_user_author"=> array("type"=> "integer:User:user/class/user.class.php", "label"=> $langs->trans("Fkuserauthor"),'enabled' => 1, 'visible' => '1', "position" => 0 ),
...
$permissiontoread= !empty($user->rights->demandestock->lire) ? $user->rights->demandestock->lire : 0;
$permissiontoadd= !empty($user->rights->demandestock->creer) ? $user->rights->demandestock->creer : 0;
$permissiontodelete= !empty($user->rights->demandestock->delete) ? $user->rights->demandestock->delete : 0;
...
if (!isModEnabled("demandestock")) {
	accessforbidden('Module demandestock not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Demandestock';
	$objectlabel = 'Demandestock';
	$uploaddir = $conf->demandestock->dir_output;
...
$title = $langs->trans("demandestock");
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/demandestock/card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-demandestock page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/demandestock/card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SenddemandestockRef";
$modelmail = "demandestock";
$objecttmp = new Demandestock($db);
...
while ($i < $imaxinloop) {
	...
	$object->status = $object->fk_statut;
	...
	if ($mode == 'kanban') {
		...
		///////////////////////////////////////////////////  fix the type_demande
		foreach ($object->fields as $key => $val) {
			if ($key == "type_demande") {
				echo "<td>";
				print $object->showOutputField($val, $key, get_ds_type()[$object->$key], '');
				echo "</td>";
				continue;
			}
